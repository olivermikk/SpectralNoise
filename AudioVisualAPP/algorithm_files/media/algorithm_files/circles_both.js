(function(p) {
  let visualizerSketch = function(p) {
    ...
    p.draw = function() {
      ...
      let level = window.audioData.amplitude || 0;  // Use amplitude for shape sizes
      let dominantFreq = window.audioData.dominantFreq || 0;  // Use dominant frequency for rotation and movement

      // Combine amplitude and dominant frequency to modulate rotation and movement
      baseShapeAngle += p.map(dominantFreq, 0, 1000, 0.005, 0.05); // Control rotation speed based on frequency
      let movementFactor = p.map(dominantFreq, 20, 500, 0.5, 2) * p.map(level, 0, 0.1, 0.5, 1.5);  // Adjust movement intensity based on both frequency and amplitude

      // Define shape sizes and segments based on both amplitude and dominant frequency
      let outerSegments = p.int(p.map(dominantFreq, 20, 500, 8, 32));  // Number of segments based on frequency
      let outerShapeSize = p.map(level, 0, 0.3, 40, 200) * p.map(dominantFreq, 20, 500, 1, 2);  // Shape size based on amplitude and frequency

      let innerSegments = p.int(p.map(dominantFreq, 20, 500, 4, 8));  // Inner layer segments
      let innerShapeSize = p.map(level, 0, 0.3, 20, 100) * p.map(dominantFreq, 20, 500, 1, 1.5);  // Inner shape size
